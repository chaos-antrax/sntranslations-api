...
def translate_text_streaming(text: str, chapter_name: str, glossary: Dict[str, str]) -> Generator[str, None, None]:
    """Stream translation from OpenRouter + DeepSeek."""
...
def translate_endpoint(payload: TranslateRequest = Body(...)) -> Dict[str, Any]:
    """Non-streaming translation endpoint (kept for backward compatibility)"""
...
def translate_stream_endpoint(payload: TranslateRequest = Body(...)):
    """Streaming translation endpoint"""
    logging.info(f"[TRANSLATE_STREAM] Started streaming translation for Novel ID: {payload.novel_id}, Chapter: {payload.chapter_name}")
    ...
    def generate():
        try:
            ...
            # Stream translation content
            full_response = ""
            for chunk in translate_text_streaming(text, chapter_name, glossary):
                full_response += chunk
            ...
            # Process the full response to extract structured data
            chapter_title_match = re.search(
                r"CHAPTER_TITLE:\s*(.*?)(?=TRANSLATION:|$)", full_response, re.DOTALL
            )
            translation_match = re.search(
                r"TRANSLATION:\s*(.*?)(?=NEW_TERMS:|$)", full_response, re.DOTALL
            )
            new_terms_match = re.search(r"NEW_TERMS:\s*([\s\S]*)", full_response)

            chapter_title = (
                chapter_title_match.group(1).strip() if chapter_title_match else chapter_name
            )
            translation = (
                translation_match.group(1).strip() if translation_match else full_response
            )
            new_terms: Dict[str, str] = {}
            ...
            logging.info(f"[TRANSLATE_STREAM] Completed streaming translation for Novel ID: {novel_id}, Chapter: {chapter_name} (Added {added} new terms)")
            ...
            logging.error(f"[TRANSLATE_STREAM] Error for Novel ID {novel_id}: {e}")
